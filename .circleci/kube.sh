...
docker build --rm=false -t coderdojo/email-service:"${GIT_SHA1}-${TIMESTAMP}" .
docker login -u "$DOCKER_USER" -p "$DOCKER_PASS" -e "$DOCKER_EMAIL"
docker push coderdojo/email-service:"${GIT_SHA1}-${TIMESTAMP}"
...
./kubectl patch deployment email-service -p '{"spec":{"template":{"spec":{"containers":[{"name":"email-service","image":"coderdojo/email-service:'"${GIT_SHA1}-${TIMESTAMP}"'"}]}}}}'
